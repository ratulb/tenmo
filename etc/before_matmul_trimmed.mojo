...
struct Matmul2dBackward[dtype: DType](ImplicitlyCopyable):
    ...
    fn backward[
        ...
        if A.requires_grad:
            ...
            var ndb = grad_out.buffer.matmul_2d[tile_size=TILE_SIZE](B.buffer.transpose(IntArray(-1, -2)))
            var grad_A = Gradbox[Self.dtype](ndb^, share=False)

            _="""var grad_A = Gradbox[Self.dtype].zeros(Shape([m, n]))
                ...
                for j_tile in range(0, n, tile_size):
                    for k_tile in range(0, p, tile_size):
                        ...
                        for i in range(i_tile, i_end):
                            ...
                            # SCALAR LOOP - no SIMD vectorization over j
                            # because B[j, k] to B[j+1, k] are not contiguous
                            for j in range(j_tile, j_end):
                                ...
                                for k in range(k_tile, k_end):
                                    ...
                                    var b_jk = B_data[b_addr]

                                    accumulator += grad_ik * b_jk
        ...
        if B.requires_grad:
            ...
            var ndb = A_buffer_transposed.matmul_2d[tile_size=TILE_SIZE](grad_out.buffer)
...
struct Matmul2d[dtype: DType](ImplicitlyCopyable):
    ...
    fn forward[
        ...
        var ndb = A.buffer.matmul_2d[tile_size=TILE_SIZE](B.buffer)
...
struct MatmulNd[dtype: DType](ImplicitlyCopyable):
    ...
    fn forward[
        ...
        var ndb = A.buffer.matmul_nd[tile_size=TILE_SIZE](B.buffer)
        var C = Tensor[Self.dtype](ndb^, requires_grad=False)

        _="""MatrixShapeValidator.validate_matrix_shapes_nd(A_shape, B_shape)
