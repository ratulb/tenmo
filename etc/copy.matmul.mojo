...
from std.sys.info import num_logical_cores as num_physical_cores
...
struct Matmul2dBackward[dtype: DType](ImplicitlyCopyable):
    ...
    fn backward[
        ...
        if A.requires_grad:
            ...
            # parallelize[process_row_tile_A](num_tiles_i, 2)
        ...
        if B.requires_grad:
            ...
            for j in range(n):
                for k in range(p):
                    var sum_val = Scalar[Self.dtype](0)

                    for i in range(m):
                        var a_idx = i * A_stride0 + j * A_stride1 + A_offset
                        var grad_idx = (
                            i * grad_out_stride0 + k * grad_out_stride1
                        )

                        if j == 0 and k == 0:  # Debug first element
                            _ = """print("i=", i, "a_idx=", a_idx, "A[i,j]=", A_data[a_idx],
                                  "grad_idx=", grad_idx, "grad[i,k]=", grad_out_data[grad_idx])"""

                        sum_val += A_data[a_idx] * grad_out_data[grad_idx]

                    var grad_B_idx = j * grad_B_stride0 + k * grad_B_stride1
                    grad_B_data[grad_B_idx] = sum_val

                    _ = """if j == 0 and k == 0:
                        print("grad_B[0,0] final =", sum_val)"""

            # print("grad_B (pointer version)")
            # grad_B.print()

            result.append((B^, grad_B^, AddTensor))
            _ = """var grad_B = Gradbox[Self.dtype].zeros(Shape([n, p]))

            # ULTRA SIMPLE - NO OPTIMIZATION
            ...
            for j in range(n):
                for k in range(p):
                    ...
                    for i in range(m):
                        # Direct tensor access - no pointer arithmetic
                        var a_val = A[i, j]  # Use your Tensor __getitem__
                        var grad_val = grad_out[i, k]
                        sum_val += a_val * grad_val

                    # Direct assignment
                    grad_B[j, k] = sum_val

            print("grad_B (simple version)")
            grad_B.print()
            ...
            _ = """var A_stride0 = A.buffer.strides[0]
            var A_stride1 = A.buffer.strides[1]
            var A_offset = A.buffer.offset
            var A_data = A.buffer.buffer.data
            ...
            parallelize[process_row_tile_B](num_tiles_j, 1)
