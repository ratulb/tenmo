...
struct CrossEntropyBackward[dtype: DType](
    ...
    fn _backward_class_indices(
        ...
        var rank = logits.rank()
        if rank == 3:
            var logits_transposed = logits.transpose[track_grad=False](0, 2, 1)
            logits_2d = logits_transposed.reshape[track_grad=False](Shape([M, C]))
        else:
        ...
        var final_grad_input: Gradbox[Self.dtype]
        if rank == 3:
            final_grad_input = grad_input_2d.reshape(Shape([N, total_spatial, C]))
            final_grad_input = final_grad_input.transpose(IntArray(0, 2, 1))
        else:
            final_grad_input = grad_input_2d.reshape(logits_shape)
