...
fn train_cifar_10() raises:
    ...
    print("  [TIME] Data loading:", Float64(t1 - t0) / 1e9, "s")
    ...
    t1 = perf_counter_ns()
    print("[TIME] NumPy to Mojo conversion:", Float64(t1 - t0) / 1e9, "s")
    ...
    t1 = perf_counter_ns()
    print("[TIME] Normalization:", Float64(t1 - t0) / 1e9, "s")
    ...
    t1 = perf_counter_ns()
    print("[TIME] Reshape:", Float64(t1 - t0) / 1e9, "s\n")
    ...
    t1 = perf_counter_ns()
    print("[TIME] DataLoader creation:", Float64(t1 - t0) / 1e9, "s")
    print("DataLoaders:")
    ...
    t1 = perf_counter_ns()
    print("  [TIME] Model construction:", Float64(t1 - t0) / 1e9, "s")
    ...
    for epoch in range(num_epochs):
        ...
        while train_loader.__has_next__():
            ...
            t1 = perf_counter_ns()
            train_forward_time += Float64(t1 - t0) / 1e9
            ...
            loss.backward()
            t1 = perf_counter_ns()
            train_backward_time += Float64(t1 - t0) / 1e9
