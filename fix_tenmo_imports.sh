...
# Build a sed expression for each module
SED_ARGS=()
for mod in "${MODULES[@]}"; do
    # Match: "from" + whitespace + module name + whitespace + "import"
    # Replace: insert dot before module name
    SED_ARGS+=(-e "s/^\\(from[[:space:]][[:space:]]*\\)\\(${mod}[[:space:]][[:space:]]*import\\)/\\1.\\2/g")
done

# Find all .mojo files under tenmo/ package dir (adjust path if needed)
find tenmo -name "*.mojo" | while read -r f; do
    echo "Processing: $f"
    sed -i "${SED_ARGS[@]}" "$f"
done
