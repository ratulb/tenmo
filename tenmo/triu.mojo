...
@fieldwise_init
struct TriuBackward[dtype: DType](ImplicitlyCopyable, RegisterPassable):
    @staticmethod
    def backward(
...
@fieldwise_init
struct Triu[dtype: DType](ImplicitlyCopyable, RegisterPassable):
    @staticmethod
    def forward[
        ...
        if rank < 2:
            panic(
                "triu requires at least 2 dimensions, got rank "
                + String(rank)
            )
        var M = shape[rank - 2]
        var N = shape[rank - 1]
        ...
        comptime if has_accelerator():
            if self.buffer.is_on_gpu():
                try:
                    ndb = TriuGpuKernel[Self.dtype].launch(
                        self.buffer, diagonal, sync
                    )
                except e:
                    panic(
                        "triu GPU forward failed: " + String(e)
                    )
                    ndb = NDBuffer[Self.dtype].Empty()
            else:
                ndb = apply_triu_cpu[Self.dtype](
                    self.buffer, M, N, diagonal
                )
        else:
            ndb = apply_triu_cpu[Self.dtype](self.buffer, M, N, diagonal)
