...
def tril_kernel[
    ...
    """Fused tril: result[i] = A[i] if col <= row + diagonal else 0."""
...
def tril_backward_kernel[
    ...
    """Backward: grad_input = grad_output * tril_mask. Same mask as forward."""
    var tid = thread_idx.x
    var gtid = tid + block_dim.x * block_idx.x
    var stride = block_dim.x * grid_dim.x
    comptime CHUNK_SIZE = simd_vectors_per_thread * simd_width
...
struct TrilGpuKernel[dtype: DType](ImplicitlyCopyable & Movable):
    ...
    def launch(
        ...
        var compiled = device_context.compile_function[
            tril_kernel[Self.datatype, simdwidth, 2 * simdwidth],
        ]()
    ...
    def launch_backward(
        ...
        var compiled = device_context.compile_function[
            tril_backward_kernel[Self.datatype, simdwidth, 2 * simdwidth],
        ]()
        device_context.enqueue_function(
            compiled,
            result_buffer,
