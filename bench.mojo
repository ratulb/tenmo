...
fn main() raises:
    A_rows, A_cols = 512, 512
    B_rows, B_cols = 512, 512

    A = Tensor.rand(Shape(A_rows, A_cols))
    B = Tensor.rand(Shape(B_rows, B_cols))
    #A = A.into_view()
    #B = B.into_view()
    output  = mm_naive(A.copy(), B.copy())
    t1 = output[1]
    expected = output[0].copy()
    output = bench_tensor_tensor(A.copy(), B.copy())
    t2 = output[1]
    result = output[0].copy()
    assert_true(expected.all_close(result))
    print("Naive vs tensor matmul without backward pass: ", t1/t2)
    # backward
    A.requires_grad_()
    B.requires_grad_()
    output_next  = bench_tensor_tensor_backward(A.copy(), B.copy())
    t3 = output_next[3]
    result = output_next[0].copy()
    assert_true(expected.all_close(result))
...
fn bench_tensor_tensor_backward[
    ...
    A_grad = A.grad().copy()
    return C^, A_grad^, B.grad().copy(), (end - start)
...
fn mm_naive[
    ...
    for m in range(M):
        for n in range(N):
            for k in range(K):
                #C[m, n] += A[m, k] * B[k, n]
                C_data = C.load[simdwidth=1](m, n)
                C.store[simdwidth=1](m, n, (C_data+ (A.load[simdwidth=1](m, k) * B.load[simdwidth=1](k, n))))
    end = perf_counter_ns()
