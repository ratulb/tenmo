...
struct MaxPool2dBackward[dtype: DType](ImplicitlyCopyable & Movable):
    """
    Optimized backward pass for MaxPool2d.
...
struct MaxPool2d[dtype: DType](ImplicitlyCopyable):
    ...
    fn forward[
        ...
        # OPTIMIZATION: Specialize for common kernel sizes
        if KH == 2 and KW == 2:
...
@fieldwise_init
struct _MaxPool2dBackward[dtype: DType](ImplicitlyCopyable & Movable):
    """
...
@fieldwise_init
@register_passable
struct _MaxPool2d[dtype: DType](ImplicitlyCopyable):
    """
    Batched, multi-channel 2D Max Pooling.
    ...
    fn forward[
        ...
        if track_grad:
            ...
            if grad_required:
                output.requires_grad_(True)
                _ = """var backward_fn = _MaxPool2dBackward[Self.dtype](
                    kernel_size=kernel_size,
                    stride=s,
                    padding=pad,
                    input_shape=input_shape,
