...
def main():
    file_data = OrderedDict()  # basename -> {imports, aliases, helpers, functions}
    ...
    with open(OUTPUT, "w") as f:
        f.write('"""Auto-generated GPU test suite.\n')
        f.write("Generated by scripts/generate_gpu_test_suite.py\n")
        total_funcs = sum(len(d["functions"]) for d in file_data.values())
        f.write(f"Contains {total_funcs} GPU-guarded test functions from {len(file_data)} files.\n")
        f.write('"""\n\n')
        ...
        # Write imports, skipping those that conflict with helper names
        for imp in all_imports:
            normalized = re.sub(r"\s+", " ", imp)
            if normalized in std_norm:
                continue
            # Check if this import introduces names that are defined by helpers
        ...
        for bname, data in file_data.items():
            ...
            # Write per-file aliases (e.g. comptime tol = ...)
            if data["aliases"]:
                for a in data["aliases"]:
                    f.write(a + "\n")
                f.write("\n")

            # Write helpers, skipping known problematic ones
    ...
    print(f"  Source files: {len(file_data)}")
