...
fn test_xor():
    ...
    var xavier = False
    _ = """model.append(
        Linear[DType.float32](2, 4, init_seed=123, xavier=xavier).into(),
        ReLU[DType.float32]().into(),
        Linear[DType.float32](4, 1, init_seed=456, xavier=xavier).into()
    )"""

    var seed1, seed2, seed3 = None, None, None

    model.append(
        Linear[DType.float32](
            2, 8, init_seed=seed1, xavier=xavier
        ).into(),  # More neurons (4→8)
        ReLU[DType.float32]().into(),
        Linear[DType.float32](
            8, 8, init_seed=seed2, xavier=xavier
        ).into(),  # Add another hidden layer
        ReLU[DType.float32]().into(),
        Linear[DType.float32](8, 1, init_seed=seed3, xavier=xavier).into(),
    )
    ...
    for epoch in range(2000):
        ...
        if epoch % 100 == 0:
            var grad_norm: Scalar[DType.float32] = Scalar[DType.float32](0.0)
...
fn test_xor_sigmoid() -> Scalar[DType.float32]:
    seed()  # Fresh random seed

    var model = Sequential[DType.float32]()
    model.append(
        Linear[DType.float32](2, 4, xavier=True).into(),  # Xavier for sigmoid!
        Sigmoid[DType.float32]().into(),
        Linear[DType.float32](4, 1, xavier=True).into(),
        Sigmoid[DType.float32]().into(),  # Output sigmoid for [0,1] range
    )
    ...
    var optimizer = SGD(
        model.parameters(), lr=0.5, momentum=0.9  # Higher LR works with sigmoid
    )
    var final_loss: Scalar[DType.float32] = 0

    for epoch in range(2000):
        var pred = model(X)
        var loss = pred.mse(y)
        final_loss = loss.sum().item()
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        if epoch % 200 == 0:
            var grad_norm: Scalar[DType.float32] = 0.0
            for param in model.parameters():
                if param[].has_grad():
                    var g = param[].grad()
                    grad_norm += (g * g).sum().item()

            # print("Epoch", epoch, "Loss:", loss.item(), "Grad norm:", sqrt(grad_norm))

            # Show predictions
            if epoch % 1000 == 0:
                print("Predictions at epoch: ", epoch)
                pred.print()
                print()
...
fn test_consistency():
    ...
    for run in range(total):
        seed()  # Fresh random init
        var final_loss = test_xor_sigmoid()  # Returns final loss

        if final_loss < 0.01:
            successes += 1
            print("Run", run, "✅ Success")
        else:
...
fn main_2():
    ...
    _ = """a = Tensor.ones(2, 3, requires_grad=True)
    ...
    sgd.zero_grad()
    print("\na's and b's grad post zero\n")
    a.grad().print()
    print()
    b.grad().print()

    # Sanity check: directly viewing params inside optimizer
    sgd.parameters[0][].print()
    print()
    sgd.parameters[1][].print()"""
...
fn test_xor_tanh():
    ...
    model.append(
        Linear[DType.float32](2, 4, xavier=True).into(),
        Tanh[DType.float32]().into(),  # ← Better than sigmoid!
        Linear[DType.float32](4, 1, xavier=True).into()
        # No output activation - raw logits
    )

    # Adjust targets to [-1, 1] range for tanh
    var y = Tensor[DType.float32].d2([[-1], [1], [1], [-1]])  # ← Not [0,1]

    var X = Tensor[DType.float32].d2([[0, 0], [0, 1], [1, 0], [1, 1]])

    var optimizer = SGD(
        model.parameters(), lr=0.1, momentum=0.9  # Higher LR works with sigmoid
    )
...
fn test_xor_with_explicit_control():
    seed()
    ...
    for epoch in range(2000):
        var pred = model(X)  # All operations use track_grad=True
        var loss = criterion(pred, y)  # MSE uses track_grad=True
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    var test_pred = model(X)  # All operations use track_grad=False
    var test_loss = criterion(test_pred, y)  # MSE uses track_grad=False

    print("\nFinal predictions (eval mode):")
    test_pred.print()
