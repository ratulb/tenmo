...
def test_uop_sqrt_forward_3d_cpu() raises:
    comptime dtype = DType.float32
    var a = Tensor[dtype].full(Shape(2, 3, 4), 4.0)
    var result = a.sqrt()
    assert_true(result.all_close(Tensor[dtype].full(Shape(2, 3, 4), 2.0)))
...
def test_uop_sqrt_backward_1d_cpu() raises:
    ...
    loss.backward()
    assert_true(
        a.grad().all_close[atol=1e-5](Tensor[dtype].d1([0.25, 0.16667, 0.125]))
    )
...
def test_uop_sqrt_grad_flow_cpu() raises:
    ...
    loss.backward()
    assert_true(a.grad().all_close[atol=1e-5](Tensor[dtype].d1([0.5, 0.33333])))
...
def test_uop_negate_forward_3d_cpu() raises:
    ...
    var result = -a
    assert_true(result.all_close(Tensor[dtype].full(Shape(2, 3, 4), -5.0)))
...
def test_uop_negate_backward_1d_cpu() raises:
    ...
    var loss = (-a).sum()
    loss.backward()
    assert_true(a.grad().all_close(Tensor[dtype].full(Shape(3), -1.0)))
...
def test_uop_negate_grad_flow_cpu() raises:
    ...
    var loss = (-a + a).sum()
    loss.backward()
    assert_true(a.grad().all_close(Tensor[dtype].full(Shape(3), 0.0)))
...
def test_uop_relu_forward_1d_cpu() raises:
    ...
    var result = a.relu()
    assert_true(result.all_close(Tensor[dtype].d1([0.0, 0.0, 0.0, 1.0, 2.0])))
...
def test_uop_relu_forward_3d_cpu() raises:
    comptime dtype = DType.float32
    var a = Tensor[dtype].full(Shape(2, 3, 4), -1.0)
    var result = a.relu()
    assert_true(result.all_close(Tensor[dtype].full(Shape(2, 3, 4), 0.0)))
...
def test_uop_relu_backward_1d_cpu() raises:
    ...
    var loss = a.relu().sum()
    loss.backward()
    assert_true(a.grad().all_close(Tensor[dtype].d1([0.0, 0.0, 1.0, 1.0])))
...
def test_uop_relu_grad_flow_cpu() raises:
    ...
    loss.backward()
    assert_true(a.grad().all_close(Tensor[dtype].d1([0.0, 2.0, 0.0, 2.0])))
...
def test_uop_invert_bool_forward_1d_cpu() raises:
    ...
    var result = ~a
    assert_true(result == Tensor[DType.bool].d1([False, True, False, False]))


def test_uop_invert_bool_forward_2d_cpu() raises:
    var a = Tensor[DType.bool].d2([[True, False], [False, True]])
    var result = ~a
    assert_true(result == Tensor[DType.bool].d2([[False, True], [True, False]]))


def test_uop_invert_bool_forward_3d_cpu() raises:
    var a = Tensor[DType.bool].full(Shape(2, 3, 4), Scalar[DType.bool](True))
    var result = ~a
    assert_true(
        result
        == Tensor[DType.bool].full(Shape(2, 3, 4), Scalar[DType.bool](False))
    )
...
def test_uop_invert_int_forward_3d_cpu() raises:
    comptime dtype = DType.int32
    var a = Tensor[dtype].full(Shape(2, 3, 4), 1)
    var result = ~a
    assert_true(result == Tensor[dtype].full(Shape(2, 3, 4), -2))
...
def test_uop_cross_relu_negate_grad_flow_cpu() raises:
    ...
    var loss = (-a).relu().sum()
    loss.backward()
    assert_true(a.grad().all_close(Tensor[dtype].d1([-1.0, 0.0, -1.0, 0.0])))
