from tenmo.tensor import Tensor
from tenmo.shapes import Shape
from std.testing import assert_true, assert_raises, TestSuite
from tenmo.broadcasthelper import ShapeBroadcaster
from tenmo.intarray import IntArray


def test_empty_shape_broadcastable() raises:
    print("test_empty_shape_broadcastable")
    shape = Shape()
    assert_true(
        ShapeBroadcaster.broadcastable(shape, Shape(1)),
        "broadcastable assertion 1 failed for empty shape",
    )
    assert_true(
        ShapeBroadcaster.broadcastable(Shape(1), shape),
        "broadcastable assertion 2 failed for empty shape",
    )
    broadcast_shape = ShapeBroadcaster.broadcast_shape(shape, Shape(1))
    assert_true(
        broadcast_shape == Shape(1),
        "Empty shape broadcast to Shape(1) assertion failed",
    )

    broadcast_shape = ShapeBroadcaster.broadcast_shape(Shape(1), shape)
    assert_true(
        broadcast_shape == Shape(1),
        "Shape(1) broadcast with empty shape assertion failed",
    )
    broadcast_mask = ShapeBroadcaster.broadcast_mask(shape, Shape(1))
    assert_true(
        IntArray(1) == broadcast_mask,
        "Empty shape broadcast mask assertion failed",
    )


def test_broadcast_shape() raises:
    print("test_broadcast_shape")
    shape1 = Shape(32, 16)
    shape2 = Shape(
        16,
    )
    result = ShapeBroadcaster.broadcast_shape(shape1, shape2)
    assert_true(result == Shape(32, 16), "Shape broadcast 1 assertion failed")

    shape1 = Shape(4, 16, 32)
    shape2 = Shape(
        32,
    )
    result = ShapeBroadcaster.broadcast_shape(shape1, shape2)
    assert_true(
        result == Shape(4, 16, 32), "Shape broadcast 2 assertion failed"
    )

    shape1 = Shape(4, 16, 32, 64)
    shape2 = Shape(
        64,
    )
    result = ShapeBroadcaster.broadcast_shape(shape1, shape2)
    assert_true(
        result == Shape(4, 16, 32, 64), "Shape broadcast 3 assertion failed"
    )

    shape1 = Shape(1)
    shape2 = Shape(
        3,
        4,
    )
    result = ShapeBroadcaster.broadcast_shape(shape1, shape2)
    assert_true(result == Shape(3, 4), "Shape broadcast 4 assertion failed")

    result = ShapeBroadcaster.broadcast_shape(Shape(2, 1), Shape(4, 2, 5))
    assert_true(result == Shape(4, 2, 5), "Shape broadcast 5 assertion failed")


def test_broadcastable() raises:
    print("test_broadcastable")
    comptime dtype = DType.float32
    assert_true(
        ShapeBroadcaster.broadcastable(Shape(1), Shape(1)),
        "broadcastable assertion 1 failed",
    )
    assert_true(
        ShapeBroadcaster.broadcastable(Shape(4, 5), Shape(1)),
        "broadcastable assertion 2 failed",
    )
    assert_true(
        ShapeBroadcaster.broadcastable(Shape(2, 3, 5), Shape(1, 5)),
        "broadcastable assertion 3 failed",
    )
    assert_true(
        ShapeBroadcaster.broadcastable(Shape(2, 3, 5), Shape(3, 5)),
        "broadcastable assertion 4 failed",
    )
    tensor1 = Tensor[dtype].d1([1, 2, 3, 4, 5])
    tensor2 = Tensor[dtype].d1([6])
    assert_true(
        tensor1.broadcastable(tensor2) and tensor2.broadcastable(tensor1),
        "Tensor shape broadcastable assertion failed",
    )


def test_pad_shapes() raises:
    print("test_pad_shapes")
    shape1 = Shape(3, 4)
    shape2 = Shape(
        4,
    )
    padded1, padded2 = ShapeBroadcaster.pad_shapes(shape1, shape2)
    assert_true(
        padded1 == shape1 and padded2 == Shape(1, 4),
        "Padding of shapes (3,4) and (4,) assertion failed",
    )
    shape1 = Shape(5, 3, 1)
    shape2 = Shape(5, 1)
    padded1, padded2 = ShapeBroadcaster.pad_shapes(shape1, shape2)
    assert_true(
        padded1 == shape1 and padded2 == Shape(1, 5, 1),
        "Padding of shapes (5,3,1) and (5,1) assertion failed",
    )
    shape1 = Shape(
        1,
    )
    shape2 = Shape(3, 4)
    padded1, padded2 = ShapeBroadcaster.pad_shapes(shape1, shape2)
    assert_true(
        padded1 == Shape(1, 1) and padded2 == shape2,
        "Padding of shapes (1, ) and (3,4) assertion failed",
    )
    shape1 = Shape(3, 4, 5, 2)
    shape2 = Shape(3, 4, 5, 2)
    padded1, padded2 = ShapeBroadcaster.pad_shapes(shape1, shape2)
    assert_true(
        padded1 == shape1 and padded2 == shape2,
        "Padding of shapes (3,4,5,2 ) and (3,4,5,2) assertion failed",
    )


def main() raises:
    TestSuite.discover_tests[__functions_in_module()]().run()
    print("\nAll broadcaster tests passed!")
