...
def test_sig_cpu_2d_forward() raises:
    ...
    var y = x.sigmoid[track_grad=False]()
    var expected = Tensor[F32].d2([[0.5, 0.7310586], [0.2689414, 0.8807970]])
    assert_true(y.all_close[atol=1e-5](expected))
...
def test_sig_cpu_2d_backward() raises:
    ...
    var loss = y.sum()
    loss.backward()
    var s = Tensor[F32].d2([[0.5, 0.8807970], [0.1192030, 0.5]])
    var expected_grad = s * (Tensor[F32].ones_like(s) - s)
    assert_true(x.grad().all_close[atol=1e-5](expected_grad))
...
def test_sig_cpu_grad_chained_with_add() raises:
    ...
    var expected = (
        (Tensor[F32].ones_like(s) - s) * s * Tensor[F32].d1([2.0, 2.0])
    )
...
def test_sig_cpu_grad_chained_with_mul() raises:
    ...
    var expected = (
        Tensor[F32].d1([3.0, 3.0]) * s * (Tensor[F32].ones_like(s) - s)
    )
    assert_true(x.grad().all_close[atol=1e-5](expected))
...
def test_sig_cpu_grad_double_sigmoid() raises:
    ...
    assert_true(x.grad().all_close[atol=1e-5](Tensor[F32].d1([expected_val])))
