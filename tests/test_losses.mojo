...
fn test_bce_loss_no_grad_mode() raises:
    ...
    assert_true(
        not loss.has_backward_fn_arg(),
        "BCE should not build graph in no_grad mode",
    )
...
fn test_bce_loss_struct_train_eval() raises:
    ...
    assert_true(
        train_loss.has_backward_fn_arg(), "BCE should build graph in train mode"
    )
    ...
    assert_true(
        not eval_loss.has_backward_fn_arg(),
...
fn test_bce_with_logits_struct() raises:
    ...
    assert_true(
        not eval_loss.has_backward_fn_arg(),
        "Should not build graph in eval mode",
    )
