...
struct IndexCalculator:
    ...
    fn flatten_index(
        ...
        if indices.size() != shape.rank():
            panic(
                (
                    "IndexCalculator → flatten_index(IntList): number of indices does not"
                    " match"
                ),
                " shape rank",
                ": len indices →",
        ...
        for dim_idx in range(indices.size()):
            ...
            if idx < 0 or idx >= dim_size:
                panic(
                    "IndexCalculator → flatten_index(IntList): index out of bounds: axis",
                    dim_idx.__str__(),
                    ", got",
    ...
    fn flatten_index(
        ...
        if indices.size() != shape.rank():
            panic(
                (
                    "IndexCalculator → flatten_index[IntArray]: number of indices does"
                    " not match"
                ),
                " shape rank",
                ": len indices →",
        ...
        for dim_idx in range(indices.size()):
            ...
            if idx < 0 or idx >= dim_size:
                panic(
                    (
                        "IndexCalculator → flatten_index[IntArray]: index out of bounds:"
                        " axis"
                    ),
                    dim_idx.__str__(),
                    ", got",
    ...
    fn flatten_index(
        ...
        if len(indices) != shape.rank():
            panic(
                (
                    "IndexCalculator → flatten_index(List[Int]): number of indices does"
                    " not match"
                ),
                " shape rank",
                ": indices →",
        ...
        for dim_idx in range(len(indices)):
            ...
            if idx < 0 or idx >= dim_size:
                panic(
                    (
                        "IndexCalculator → flatten_index(List[Int]): index out of bounds:"
                        " axis"
                    ),
                    dim_idx.__str__(),
                    ", got",
