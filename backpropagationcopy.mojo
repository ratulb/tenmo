...
from viewbackwardcopy import ViewBackward
...
struct BackwardFn[dtype: DType](Copyable & Movable):
    ...
    fn __call__(
        self, output_ptr: UnsafePointer[TensorLite[dtype]]
    ) -> List[Tuple[TensorLite[dtype], Tensor[dtype], Int]]:
        _="""if self.grad_fn.isa[MatmulBackward[dtype]]():
            return self.grad_fn[MatmulBackward[dtype]].backward[dtype](output_ptr)

        if self.grad_fn.isa[TensorViewBackward[dtype]]():
            return self.grad_fn[TensorViewBackward[dtype]].backward[dtype](output_ptr)

        if self.grad_fn.isa[PermuteBackward[dtype]]():
            return self.grad_fn[PermuteBackward[dtype]].backward[dtype](output_ptr)

        elif self.grad_fn.isa[SumBackward[dtype]]():
            return self.grad_fn[SumBackward[dtype]].backward[dtype](output_ptr)

        elif self.grad_fn.isa[MeanBackward[dtype]]():
            return self.grad_fn[MeanBackward[dtype]].backward[dtype](output_ptr)

        elif self.grad_fn.isa[AddBackward[dtype]]():
            return self.grad_fn[AddBackward[dtype]].backward[dtype](output_ptr)
            ...
            return self.grad_fn[SubBackward[dtype]].backward[dtype](output_ptr)
            ...
            return self.grad_fn[TBackward[dtype]].backward[dtype](output_ptr)

        elif self.grad_fn.isa[TransposeBackward[dtype]]():
            return self.grad_fn[TransposeBackward[dtype]].backward[dtype](output_ptr)
        ...
        if self.grad_fn.isa[ViewBackward[dtype]]():
            return self.grad_fn[ViewBackward[dtype]].backward[dtype](output_ptr)


        if self.grad_fn.isa[ReshapeBackward[dtype]]():
            return self.grad_fn[ReshapeBackward[dtype]].backward[dtype](output_ptr)
